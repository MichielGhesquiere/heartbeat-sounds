...
class FeatureDataset(Dataset):
    def __init__(self, X, y, mode: str, sr: int, n_mfcc: int = 40, seq_mode: str = 'seq_mel_mfcc',
                 n_mels: int = 64, add_deltas: bool = True, add_stats: bool = True):
        ...
        if mode == 'mfcc':
            ...
            seq_feats, _, feature_dim = batch_extract_sequence(
                X, sr=sr, mode=seq_mode, n_mels=n_mels, n_mfcc=n_mfcc,
                add_deltas=add_deltas, add_spectral_stats=add_stats
            )
...
def run_training(data_dir: str = 'data', results_dir: str = 'results', models_dir: str = 'models',
                 ...
                 feature_type: str = 'sequence', seq_mode: str = 'seq_mel_mfcc', n_mels: int = 64,
                 add_deltas: bool = True, add_stats: bool = True, class_weighting: bool = True,
                 focal_loss: bool = False, gamma: float = 2.0,
                 save_experiment: bool = True, experiment_label: str | None = None):
    ...
    train_ds = FeatureDataset(X_train, y_train, mode='mfcc' if feature_type=='mfcc' else 'sequence', sr=sr, n_mfcc=n_mfcc,
                              seq_mode=seq_mode, n_mels=n_mels, add_deltas=add_deltas, add_stats=add_stats)
    val_ds = FeatureDataset(X_val, y_val, mode='mfcc' if feature_type=='mfcc' else 'sequence', sr=sr, n_mfcc=n_mfcc,
                            seq_mode=seq_mode, n_mels=n_mels, add_deltas=add_deltas, add_stats=add_stats)
    test_ds = FeatureDataset(X_test, y_test, mode='mfcc' if feature_type=='mfcc' else 'sequence', sr=sr, n_mfcc=n_mfcc,
                             seq_mode=seq_mode, n_mels=n_mels, add_deltas=add_deltas, add_stats=add_stats)
    ...
    if feature_type == 'mfcc':
        ...
        feature_dim = getattr(train_ds, 'feature_dim')
        model = build_sequence_model(feature_dim=feature_dim, num_classes=len(CATEGORY_MAP)).to(device)

    # Class weighting
    if class_weighting:
        class_counts = np.bincount(y_train, minlength=len(CATEGORY_MAP))
        inv_freq = class_counts.max() / (class_counts + 1e-6)
        weights = torch.tensor(inv_freq / inv_freq.sum() * len(CATEGORY_MAP), dtype=torch.float32, device=device)
    else:
        weights = None
    ...
    # Load best
    best_model_load_path = os.path.join(exp_models_dir if save_experiment else models_dir, 'best_model.pt')
    ...
    report = classification_report(y_all, preds_all, target_names=list(CATEGORY_MAP.keys()), output_dict=True)  # type: ignore[assignment]
    report_path = os.path.join(exp_results_dir if save_experiment else results_dir, 'classification_report.txt')
        ...
        with open(report_path, 'w') as f:
            for cls, stats in rep_dict.items():  # type: ignore[attr-defined]
                if isinstance(stats, dict) and 'precision' in stats:
                    f.write(f"{cls}: precision={stats['precision']:.3f} recall={stats['recall']:.3f} f1={stats['f1-score']:.3f} support={stats['support']}\n")
                else:
                    f.write(f"{cls}: {stats}\n")
    ...
    np.savez(os.path.join(exp_results_dir if save_experiment else results_dir, 'training_history.npz'),
    ...
    if save_experiment:
        per_class_recalls = [rep_dict[name]['recall'] if name in rep_dict else float('nan') for name in CATEGORY_MAP.keys()]  # type: ignore[index]
        # Build label
        if experiment_label is None:
        ...
        exp_csv = os.path.join(results_dir, 'experiment_recalls_detailed.csv')  # keep global aggregation at root
        header = 'experiment_label,feature_type,seq_mode,class_weighting,focal_loss,add_stats,add_deltas,' + ','.join([f"recall_{k}" for k in CATEGORY_MAP.keys()]) + '\n'
        line = (
    ...
    plt.tight_layout()
    cm_path = os.path.join(exp_results_dir if save_experiment else results_dir, 'confusion_matrix.png')
    ...
    plt.tight_layout()
    curves_path = os.path.join(exp_results_dir if save_experiment else results_dir, 'training_curves.png')
...
def run_experiment_suite(data_dir: str = 'data', results_dir: str = 'results', models_dir: str = 'models', **common):
    ...
    """
    experiments = [
        dict(feature_type='mfcc', class_weighting=False, add_deltas=False, add_stats=False, experiment_label='1_mfcc_baseline'),
        dict(feature_type='sequence', class_weighting=False, add_deltas=False, add_stats=False, experiment_label='2_seq_base'),
        dict(feature_type='sequence', class_weighting=True, add_deltas=False, add_stats=False, experiment_label='3_seq_cw'),
        dict(feature_type='sequence', class_weighting=True, add_deltas=True, add_stats=True, experiment_label='4_seq_cw_deltas_stats'),
    ]
    results = []
    ...
    try:
        ...
        if os.path.exists(csv_path):
            ...
            if rows:
                ...
                deltas = {cls: [float(r[indices[cls]]) - float(base[indices[cls]]) for r in rows] for cls in class_keys}
                plt.figure(figsize=(10,5))
